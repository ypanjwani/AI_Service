...
const okCls  = 'border-white/10 focus:border-sky-500 focus:ring-sky-500/20'
...
const FEATURES = [
  {
    ...
    desc: 'AI reads every incoming support ticket, matches it against your knowledge base and past resolutions, and closes common issues instantly — without a human ever touching them. Resolution rates of 60–80% on tier-1 queries are typical.',
    icon: (
  ...
  },
  {
    ...
    desc: 'When a query is complex, emotionally charged, or requires account-level access, the AI escalates with full context — conversation history, sentiment score, and a suggested resolution — so your agent picks up exactly where the AI left off.',
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8}
  ...
  {
    color: 'indigo',
    title: 'Omnichannel Support Coverage',
    desc: 'One AI brain handles support across your website chat, WhatsApp, email, and in-app help — with a unified conversation thread per customer. No more siloed channels or repeated context from frustrated customers.',
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8}
  ...
  },
  {
    ...
    desc: 'The AI detects frustration, urgency, and satisfaction in real time — adjusting tone and response priority accordingly. Angry customers get faster responses with empathetic language. Happy customers get upsell opportunities surfaced naturally.',
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8}
...
const colorMap = {
  sky:    { icon: 'text-sky-400',    bg: 'bg-sky-500/8 border-sky-500/15' },
...
const INDUSTRIES = [
  'SaaS & Tech', 'E-Commerce', 'Telecom', 'Banking & Finance',
  'Healthcare', 'Travel & Hospitality', 'Insurance', 'Retail',
  'HR & Recruitment', 'Real Estate', 'Government Services', 'Education',
]

const INCLUDED = [
  'AI support agent design & training',
  'Helpdesk integration (Freshdesk / Zendesk / custom)',
  'Auto ticket classification & priority routing',
  'Escalation rules & human handoff flows',
  'Knowledge base connection & sync',
  'Omnichannel deployment (chat, WhatsApp, email)',
  'Sentiment analysis & urgency detection',
  'CSAT tracking & analytics dashboard',
...
export default function CustomerSupportServicePage() {
  ...
  return (
    <div className="min-h-screen bg-slate-950">
      ...
      <div className="fixed top-0 right-1/3 w-[600px] h-[500px] bg-sky-600/5 rounded-full blur-3xl pointer-events-none z-0" />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8 pt-32 pb-28">
        ...
        <Link
          ...
          className="inline-flex items-center gap-2 text-[0.8rem] font-semibold text-slate-500 hover:text-sky-400 transition-colors duration-200 mb-12"
        >
          ...
          <div className="lg:col-span-7 space-y-14">
            ...
            <div>
              <span className="inline-flex items-center gap-2 mb-5">
                ...
                <span className="text-[0.7rem] font-black text-sky-400 uppercase tracking-[0.22em]">
                  AI Customer Support
                </span>
              </span>
              ...
              <p className="text-[1.05rem] text-slate-400 leading-[1.85] mb-5">
                We build AI-powered customer support systems that resolve the majority of tickets
                automatically — across chat, WhatsApp, and email — while routing the rest to your
                team with full context already attached. The result is faster resolutions, lower
                support costs, and happier customers.
              </p>

              <p className="text-[0.95rem] text-slate-500 leading-[1.85]">
                Whether you're handling 100 tickets a day or 10,000, the system scales without
                adding headcount. Your human agents focus on complex, high-value interactions
                while AI handles everything else — reliably, consistently, and around the clock.
              </p>
            </div>
            ...
            <div>
              ...
              <p className="text-[0.85rem] text-slate-500 mb-5 leading-relaxed">
                Any business that receives repetitive customer queries at scale is an ideal candidate.
              </p>
            ...
            <div>
              <h2 className="text-[1.2rem] font-black text-white mb-2">Platforms we integrate with</h2>
              <p className="text-[0.85rem] text-slate-500 mb-5">
                We work with your existing helpdesk — no migration required.
              </p>
              <div className="flex flex-wrap gap-3 mb-4">
                {[
                  { label: 'Freshdesk / Zendesk', sub: 'Helpdesk layer' },
                  { label: 'WhatsApp & Email', sub: 'Channels' },
                  { label: 'Claude / GPT-4o', sub: 'AI reasoning' },
                ].map(l => (
                  <div
                    key={l.label}
                    className="px-5 py-3 bg-sky-500/8 border border-sky-500/18 rounded-xl text-center min-w-[150px]"
                  >
                    <p className="text-[0.85rem] font-black text-sky-300">{l.label}</p>
                    <p className="text-[0.65rem] text-sky-500/60 mt-0.5">{l.sub}</p>
                  </div>
                ))}
              </div>
              <p className="text-[0.74rem] text-slate-600 leading-relaxed">
                Also integrates with: Intercom · HubSpot Service Hub · Salesforce Service Cloud · custom ticketing systems via REST API.
              </p>
            </div>
                  ...
                  <ul className="space-y-2.5">
                    {INCLUDED.map(item => (
                      <li key={item} className="flex items-start gap-2.5 text-[0.82rem] text-slate-300">
                        <svg className="w-3.5 h-3.5 text-sky-400 mt-0.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="px-7 py-6">
                  {!user ? (
                    <div className="text-center py-4">
                      ...
                      <button
                        onClick={() => navigate('/login', { state: { next: '/service/customer-support' } })}
                        className="w-full bg-sky-600 hover:bg-sky-500 text-white font-black text-[0.88rem] py-3 rounded-xl transition-all duration-200 shadow-lg shadow-sky-600/25 mb-3"
                      >
                        Sign In
                      </button>
                      <a href="/register" className="text-[0.78rem] text-slate-500 hover:text-sky-400 transition-colors">
                    ...
                    <form onSubmit={handleConfirm} noValidate className="space-y-5">
                      ...
                      <div className="flex items-center gap-3 p-3.5 bg-slate-800/50 border border-white/6 rounded-2xl">
                        <div className="w-10 h-10 rounded-full bg-sky-600 flex items-center justify-center text-white text-[0.85rem] font-black shrink-0 shadow-lg shadow-sky-600/30">
                      ...
                      <div>
                        ...
                        <textarea
                          rows={3}
                          placeholder="e.g. We get 500+ support tickets a day and need to automate responses for common queries…"
                          value={message}
                      ...
                      <button
                        type="submit"
                        disabled={loading}
                        className="w-full bg-sky-600 hover:bg-sky-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-black text-[0.9rem] py-3.5 rounded-xl transition-all duration-200 shadow-lg shadow-sky-600/30 hover:shadow-sky-500/40 flex items-center justify-center gap-2"
                      >
                        {loading ? (
                          <>
