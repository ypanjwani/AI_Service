...
const okCls  = 'border-white/10 focus:border-green-500 focus:ring-green-500/20'
...
const FEATURES = [
  {
    ...
    desc: 'Turn your WhatsApp into a 24/7 sales machine. AI guides prospects from first message to confirmed order — answering objections, sharing catalogues, collecting details, and closing deals without any human involvement.',
    icon: (
  ...
  {
    color: 'emerald',
    title: 'Order & Appointment Management',
    desc: 'Customers place orders, book appointments, track deliveries, and receive confirmations — all via WhatsApp. The AI handles the entire flow and syncs updates to your backend or CRM in real time.',
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8}
  ...
  },
  {
    ...
    desc: 'Every inbound WhatsApp message is analysed by AI to score intent, extract key details, and route hot leads directly to your sales team — so your team only spends time on prospects who are genuinely ready to buy.',
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8}
  ...
  {
    color: 'lime',
    title: 'Broadcast Campaigns with Personalisation',
    desc: 'Send targeted, personalised broadcast messages to segmented contact lists — promotions, reminders, restock alerts, and follow-ups. Each message is dynamically tailored per recipient for maximum open and response rates.',
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8}
...
const colorMap = {
  ...
  lime:    { icon: 'text-lime-400',    bg: 'bg-lime-500/8 border-lime-500/15' },
}

const INDUSTRIES = [
  'E-Commerce', 'Healthcare', 'Real Estate', 'Education',
  'Food & Restaurants', 'Retail', 'Finance & Insurance', 'Travel & Hospitality',
  'HR & Recruitment', 'Automotive', 'Beauty & Wellness', 'NGOs & Non-Profits',
]

const INCLUDED = [
  'WhatsApp Business API setup & verification',
  'AI conversation flow design',
  'Automated sales funnel build',
  'Order / appointment management flow',
  'CRM & backend integration',
  'Personalised broadcast campaign setup',
...
export default function WhatsAppAIPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-950">
      ...
      <div className="fixed top-0 right-1/3 w-[600px] h-[500px] bg-green-600/5 rounded-full blur-3xl pointer-events-none z-0" />
      <div className="fixed bottom-1/3 left-0 w-[350px] h-[400px] bg-emerald-700/5 rounded-full blur-3xl pointer-events-none z-0" />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8 pt-32 pb-28">
        ...
        <Link
          ...
          className="inline-flex items-center gap-2 text-[0.8rem] font-semibold text-slate-500 hover:text-green-400 transition-colors duration-200 mb-12"
        >
          ...
          <div className="lg:col-span-7 space-y-14">
            ...
            <div>
              <span className="inline-flex items-center gap-2 mb-5">
                ...
                <span className="text-[0.7rem] font-black text-green-400 uppercase tracking-[0.22em]">
              ...
              <p className="text-[1.05rem] text-slate-400 leading-[1.85] mb-5">
                Your customers are already on WhatsApp. We build AI-powered automation systems
                that handle sales conversations, qualify leads, manage orders, and run personalised
                broadcast campaigns — all without a human in the loop, 24 hours a day.
              </p>

              <p className="text-[0.95rem] text-slate-500 leading-[1.85]">
                From a local business wanting to automate customer replies to a growing e-commerce
                brand needing a full order management flow on WhatsApp, we design and build the
                entire system end-to-end — including WhatsApp Business API setup and CRM integration.
              </p>
            </div>
            ...
            <div>
              ...
              <p className="text-[0.85rem] text-slate-500 mb-5 leading-relaxed">
                Any business where customers communicate via WhatsApp is a strong fit — which in India is nearly every business.
              </p>
            ...
            <div>
              <h2 className="text-[1.2rem] font-black text-white mb-2">How we build it</h2>
              <p className="text-[0.85rem] text-slate-500 mb-5">
                We use the official WhatsApp Business API — no third-party bots, no grey-area tools.
              </p>
              <div className="flex flex-wrap gap-3 mb-4">
                {[
                  { label: 'WhatsApp Business API', sub: 'Official channel' },
                  { label: 'AI Conversation Layer', sub: 'Claude / GPT-4o' },
                  { label: 'CRM Integration', sub: 'Your existing stack' },
                ].map(l => (
                  <div
                    key={l.label}
                    className="px-5 py-3 bg-green-500/8 border border-green-500/18 rounded-xl text-center min-w-[150px]"
                  >
                    <p className="text-[0.85rem] font-black text-green-300">{l.label}</p>
                    <p className="text-[0.65rem] text-green-500/60 mt-0.5">{l.sub}</p>
                  </div>
                ))}
              </div>
              <p className="text-[0.74rem] text-slate-600 leading-relaxed">
                Integrations: HubSpot · Salesforce · Zoho · Google Sheets · Custom webhooks · REST APIs — we connect to whatever you already use.
              </p>
            </div>
                ...
                <div className="px-7 py-6">
                  {!user ? (
                    <div className="text-center py-4">
                      ...
                      <button
                        onClick={() => navigate('/login', { state: { next: '/service/whatsapp-ai' } })}
                        className="w-full bg-green-600 hover:bg-green-500 text-white font-black text-[0.88rem] py-3 rounded-xl transition-all duration-200 shadow-lg shadow-green-600/25 mb-3"
                      >
                        Sign In
                      </button>
                      <a href="/register" className="text-[0.78rem] text-slate-500 hover:text-green-400 transition-colors">
                    ...
                    <form onSubmit={handleConfirm} noValidate className="space-y-5">
                      ...
                      <div className="flex items-center gap-3 p-3.5 bg-slate-800/50 border border-white/6 rounded-2xl">
                        <div className="w-10 h-10 rounded-full bg-green-600 flex items-center justify-center text-white text-[0.85rem] font-black shrink-0 shadow-lg shadow-green-600/30">
                      ...
                      <div>
                        ...
                        <textarea
                          rows={3}
                          placeholder="e.g. I run an e-commerce store and want to automate order updates and customer follow-ups on WhatsApp…"
                          value={message}
                      ...
                      <button
                        type="submit"
                        disabled={loading}
                        className="w-full bg-green-600 hover:bg-green-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-black text-[0.9rem] py-3.5 rounded-xl transition-all duration-200 shadow-lg shadow-green-600/30 hover:shadow-green-500/40 flex items-center justify-center gap-2"
                      >
                        {loading ? (
                          <>
